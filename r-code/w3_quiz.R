...
pv1 <- NPV(cost, rep(375, 2), r, c(5,11)); pv1

# Option 2 - interest from money market account
pv2 <- -cost + FV(450, r, 11); pv2


## Question 2
# (5 points) Juanita has an opportunity to invest in her friend's clothing store. The initial investment is $10,100 and the expected annual cashflows thereafter are as follows: {$300; $800; $1,300; $2,000; $2,000; $5,000; $5,000}. What is Juanita's IRR on this investment?
irr <- IRR(10100, c(300, 800, 1300, 2000, 2000, 5000, 5000))
round(irr*100, 2)


## Question 3
# (5 points) Fabrice is looking to buy a new plug-in hybrid vehicle. The purchase price is $11,000 more than a similar conventional model. However, he will receive a $4,900 federal tax credit that he will realize at the end of the year. He estimates that he will save $1,300 per year in gas over the conventional model; these cash outflows can be assumed to occur at the end of the year. The cost of capital (or interest rate) for Fabrice is 6%. How long will Fabrice have to own the vehicle to justify the additional expense over the conventional model? In other words, what is the DISCOUNTED payback period in years? Discount future cash flows before calculating payback rounded UP to a whole year.
#
...
## Question 5
# (10 points) Da Feng is looking to refinance his home because rates have gone down since he purchased the house 5 years ago. He started with a 30-year fixed-rate mortgage of $286,000 at an annual rate of 7.10%. He has to make monthly payments. He can now get a 25-year fixed-rate mortgage at an annual rate of 4.10% on the remaining balance of his initial mortgage. This loan also requires monthly payments. In order to re-finance, Da Feng will need to pay closing costs of $4,200. These costs are out of pocket and cannot be rolled into the new mortgage. How much will refinancing save Da Feng? (i.e. What is the NPV of the refinancing decision?)
cost <- 286e3
n <- 30 * monthsPerYear
r <- 0.071 / monthsPerYear
...
# Adjust tax revenue to equal value in each year (with 3% pa increase from 1868)
taxLs <- rep(0, year.n)
for (i in 1:year.n) {
    taxLs[i] <- PV(tax2018, rTax, year.n-i)
}
...
## Question 8
# (15 points) Diane has just turned 18 and also completed high school. She is wondering about the value of a college education. She is pretty good with numbers, and driven by financial considerations only, so she sits down to calculate whether it is worth the large sum of money involved. She knows that her first year tuition will be $14,000, due at the beginning of the year (that is, right away). Based on historical trends she estimates that tuition will rise at 7% per year for the 4 years she is in school. She also estimates that her living expense above and beyond tuition will be $7,500 per year (assume this occurs at the end of the year) for the first year and will increase $500 each year thereafter to keep up with inflation. She does not plan to work at all while attending school. Were she to forgo college she would be able to make $24,000 per year out of high school and expects that to grow 4% annually. With the college degree, she estimates that she will earn $47,000 per year out of college, again with annual 4% increases in salary. Either way, she plans to work until 60 (she begins college right away). The interest/discount rate is 5%. What is the NPV of her college education? (Note: All cash flows except tuition payments occur at the end of the year.)
college.fee0 <- 14e3
college.g <- 0.07
college.n <- 4
...
#
# Opportunity cost of investment (in today's $)
r <- 0.11 / monthsPerYear
n0to1 <- 9
n1to2 <- (27 - n0to1) * monthsPerYear
n0to1 <- n0to1 * monthsPerYear

cost0 <- 260e3
cost1 <- FV(cost0, r, n0to1)

profit.c <- 3e3
profit.0to1 <- annuity_fv(profit.c, r, n0to1)
profit.1to2 <- annuity_pv(profit.c, r, n1to2)

npv <- profit.0to1 + profit.1to2 - cost1; npv
