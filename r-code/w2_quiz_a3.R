...
## Question 1
# (5 points) Carlos goes to the bank to take out a personal loan. The stated annual interest rate is 8%, but interest is compounded quarterly and he will make monthly payments. What is the EAR?
r <- 0.08
quartPerYear <- 4
EAR(r, quartPerYear)
...
## Question 6
# (10 points) Melanie and Stephen Jackson are purchasing their first house. The house costs $400,000. They have put a 10% down payment (that is, an amount that banks should require you to pay out-of-pocket), but will therefore finance the rest. They are considering a fixed rate 15-year mortgage at a 7.75% APR with monthly payments. How much will the Jacksons' first monthly payment be?
#
...
## Question 9
# (15 points) You have been living in the house you bought 6 years ago for $400,000. At that time, you took out a loan for 80% of the house at a fixed rate 20-year loan at an annual stated rate of 8.5%. You have just paid off the 72th monthly payment. Interest rates have meanwhile dropped steadily to 4.5% per year, and you think it is finally time to refinance the remaining balance over the residual loan life. But there is a catch. The fee to refinance your loan is $4,500. Should you refinance the remaining balance? How much would you save/lose if you decided to refinance?
pv <- 400e3 * 0.8
r <- 0.085 / monthsPerYear
n <- 20 * monthsPerYear
mPay <- 72
refinCost <- 4500
...
# PV of cash flows difference paid over n remaining months (in future)
...
## EXTRA FOR Q10
################
# Future value of c.cost if leave in bank
c.fv <- FV(c.cost, r, n); c.fv

# Future value of c.cost if have this money now in bank and make periodic installments
pv_credit(c.cost, c.c, r, n)$fv

# ^this equals 0 which demonstrates that c.cost MUST be pv in bank account earning interest with which we use to make the periodic payments. The reason is that in reality, assuming inflation rate < market rate (RBA interest rate), income earned in future will not rise as fast as interest accumulated from pv in bank.
